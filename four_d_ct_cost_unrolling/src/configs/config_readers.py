...
def get_default_backbone_config() -> dict:
    ...
    resource_path = '/'.join(('four_d_ct_cost_unrolling','src','configs','backbone_default_configs.json'))
...
def get_default_w_constraints_config() -> dict:
    package_name = __name__.split('.')[0]
    resource_path = '/'.join(('four_d_ct_cost_unrolling','src','configs','w_constraints_default_configs.json'))
    file_path = pkg_resources.resource_filename(package_name, resource_path)
